...
import { Calendar, MapPin, Users, Trophy, Settings, BarChart3 } from "lucide-react"
      ...
      <CardHeader className="pb-3 md:pb-4">
        <div className="flex justify-between items-start mb-2">
          ...
          <div className="flex gap-1">
            <Button size="sm" variant="ghost" onClick={() => onAnalytics(event.id)} className="h-8 w-8 p-0">
              <BarChart3 className="h-3 w-3 md:h-4 md:w-4" />
            </Button>
            <Button size="sm" variant="ghost" onClick={() => onManage(event.id)} className="h-8 w-8 p-0">
        ...
        <div className="space-y-2 md:space-y-3">
          ...
          <div className="flex flex-wrap gap-1 mt-3 md:mt-4">
            {event.tracks.slice(0, 2).map((track) => (
              <Badge key={track} variant="outline" className="text-xs truncate max-w-[80px] md:max-w-none">
                {track}
              </Badge>
            ))}
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-4 pt-2 text-center">
            <div>
              <div className="text-lg md:text-2xl font-bold text-primary">{event.participants}</div>
              <div className="text-xs text-muted-foreground">Participants</div>
            </div>
            <div>
              <div className="text-lg md:text-2xl font-bold text-secondary">{event.submissions}</div>
